---
name: jrtc-faq
description: 通过会议查询工具获取 JRTC 会议与人员信息，通过 Grafana MCP 查询丢包率、
...
- 密码、Token、API Key、Cookie 和 Authorization 值必须替换为 `[REDACTED]`。
- Skill 名称、路径、查询流程、MCP 工具名称、调用参数、查询语句、业务结果和错误详情可以正常展示。
...
会议和人员映射使用服务端自动提供的 `meeting_lookup` MCP 工具。网络请求和认证由服务端完成；
不访问旧 scheduler Elasticsearch 表。若工具不可用或返回错误，输出具体错误详情。
...
- 工具返回 `error` 表示查询失败，不等于无记录。可以输出经过认证值脱敏的完整错误详情。
